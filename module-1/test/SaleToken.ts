...
describe("SaleToken", function () {
  ...
  describe("Buying", function () {
    it("Buy SaleToken at correct rate", async function () {
      ...
      expect(await saleToken.provider.getBalance(saleToken.address)).to.equal(ethers.utils.parseEther("1"));
      expect(await saleToken.balanceOf(user1.address)).to.equal(10_000n * (10n ** 18n));
    })
  });

  describe("Withdrawing", function () {
    it("Withdraw all ether collected by SaleToken", async function () {
      const { deployer, user1, user2, saleToken } = await loadFixture(deploySaleToken);
      ...
      expect(await saleToken.provider.getBalance(user2.address)).to.equal(ethers.utils.parseEther("10001"));
    });
  });
});
